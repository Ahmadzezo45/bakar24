        ...
        await axios.get(`http://api.openweathermap.org/data/2.5/weather?q=${kota}&units=metric&appid=132d0c03530775bbbfa9b298392866df`)
            .then(async ({
                ...
                if (data.cod == 404) {
                    ...
                    sunrise = await moment.tz(data.sys.sunrise * 1000, "Africa/Cairo").format("DD, MM - yy : HH:mm") + " WIB";
                    sunset = await moment.tz(data.sys.sunset * 1000, "Africa/Cairo").format("DD, MM - yy : HH:mm") + " WIB";
                    result = {
